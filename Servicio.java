...
public class Servicio
{
  ...
  public Response alta(@FormParam("usuario") Usuario usuario) throws Exception
  {
    ...
    try
    {
      PreparedStatement stmt_1 = conexion.prepareStatement("SELECT email FROM usuarios WHERE email=?");
      try
      {
        ...
        {
          if (rs.next())
             return Response.status(400).entity(j.toJson(new Error("El usuario ya existe"))).build();
        }
        finally
        {
          rs.close();
      ...
      try
      {
        ...
        /*result = stmt_2.executeQuery();*/
        result = stmt_2.getGeneratedKeys();
        if(result.next() && result != null){
          usuario.id_usuario = result.getInt(1);
          System.out.println("Usuario creado con el ID: " + result.getInt(1));
        }else{
    ...
    return Response.status(200).entity(j.toJson(usuario)).build();
  }
