...
public class Servicio2
{
  ...
  {		
    try
    {
      ...
      pool = (DataSource)ctx.lookup("java:comp/env/jdbc/datasource_Servicio2");
    }
    catch(Exception e)
    {
      e.printStackTrace();
  ...
  public Response alta(@FormParam("usuario") Usuario usuario) throws Exception
  {
    ...
    try
    {
      ...
      try
      {
        ...
        {
          if (rs.next()){
             return Response.status(400).entity(j.toJson(new Error("El email ya fue registrado en el usuario cuyo id es: "+ rs.getString(1)))).build();}
        }
        finally
        {
          rs.close();
      ...
      try
      {
        ...
        ResultSet get_id;
        get_id = stmt_2.getGeneratedKeys();
        if(get_id.next() && get_id!= null){
          usuario.id_usuario = get_id.getInt(1);
          System.out.println("\nUsuario registrado con el ID: " + get_id.getInt(1));
        }else{
  ...
  public Response consulta(@FormParam("id_usuario") int id_usuario) throws Exception
  {
    ...
    try
    {
      PreparedStatement stmt_1 = conexion.prepareStatement("SELECT a.id_usuario,a.email,a.nombre,a.apellido_paterno,a.apellido_materno,a.fecha_nacimiento,a.telefono,a.genero,b.foto FROM usuarios a LEFT OUTER JOIN fotos_usuarios b ON a.id_usuario=b.id_usuario WHERE email=?");
      try
      {
  ...
  public Response modifica(@FormParam("usuario") Usuario usuario) throws Exception
  {
    ...
    if (usuario.id_usuario == 0)
      return Response.status(400).entity(j.toJson(new Error("Se debe ingresar el id_usuario"))).build();
